...
class FileWriterVersion_1 extends \Classes\Factory\Export\BinaryData\File\FileWriter {
    ...
    public function export($objects) {
        ...
        $sampleObject = array_shift(array_values($objects));
        $className = $this->getClassName($sampleObject);
        $docWriter = \Classes\Factory\Export\BinaryData\Document\DocumentWriterFactory::make($className);
        
        if ($docWriter instanceof \Classes\Factory\Export\BinaryData\Document\DocumentWriter) {
